...
def payment(request):
    ...
    course = batch.course
    course_fee = course.course_fee or Decimal("0")

    # ✅ Fetch Admission Fee assigned by BDM
    admission_record = FeePayment.objects.filter(
        student=student,
        payment_type=FeePayment.PaymentType.ADMISSION
    ).first()

    admission_amount = admission_record.amount if admission_record else Decimal("0")

    # ✅ Check Admission Paid
    ...
    # ✅ Check Full Payment
    full_paid = FeePayment.objects.filter(
        student=student,
        payment_type=FeePayment.PaymentType.FULL_PAYMENT,
        payment_status=FeePayment.PaymentStatus.COMPLETED
    ).exists()

    # If full payment completed → admission automatically paid
    if full_paid:
        admission_paid = True
...
def emi(request):
    ...
    # ✅ Only allow if EMI exists
    emis = FeePayment.objects.filter(
        student=student,
        payment_method=FeePayment.PaymentMethod.EMI
    ...
    if request.method == "POST":
        ...
        emi_obj = FeePayment.objects.filter(
            id=emi_id,
            student=student,
            payment_method=FeePayment.PaymentMethod.EMI
        ).first()
        ...
        if emi_obj:
            emi_obj.payment_status = FeePayment.PaymentStatus.PENDING
            emi_obj.transaction_id = f"EMI-{uuid.uuid4().hex[:8]}"
            emi_obj.payment_date = timezone.now()
            emi_obj.save()
    ...
    course_fee = batch.course.course_fee if batch else Decimal("0")
...
def installments(request):
    ...
    if batch and batch.course:
        course_name = batch.course.name   # change to .title if needed
        course_fee = batch.course.course_fee or Decimal("0")
    ...
    next_pending = installments.filter(
    ...
    return render(
        ...
        "student/payment/installments.html",
        {
            "student": student,
            "batch": batch,
            "course_name": course_name,  # 👈 sending correct variable
            "course_fee": course_fee,
...
@login_required
def install_qr(request, installment_number):
    ...
    if not installment:
        messages.error(request, "Installment not found.")
        return redirect("student:installments")
    ...
    if request.method == "POST":
        ...
        if receipt:
            ...
            messages.success(request, "Receipt uploaded successfully. Waiting for BDM verification.")
...
def payment_gateway(request):
    ...
    elif FeePayment.objects.filter(
        student=student,
        payment_method=FeePayment.PaymentMethod.EMI
    ).exists():
        return redirect("student:emi")
